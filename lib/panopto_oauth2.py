...
# This code uses this local URL as redirect target for Authorization
# Code Grant (Server-side Web Application)
...
class PanoptoOAuth2():
    def __init__(self, server, client_id, client_secret,
        ...
        self.cache_file = os.path.join(
                cache_dir,
                'token_{0}_{1}.cache'.format(server, client_id))
    ...
    def get_access_token_authorization_code_grant(self):
        ...
        with RedirectTCPServer() as httpd:
            info('HTTP server started at port {0}. Waiting for redirect.'.format(REDIRECT_PORT))
            # Serve one request.
        ...
        info('Get a new access token with authorization code, which is provided as return path: {0}'.format(redirected_path))
    ...
    def __get_refreshed_access_token(self):
        ...
        '''
        try:
            info('Read cached token from {0}'.format(self.cache_file))
            ...
            info('Get a new access token by using saved refresh token.')
        ...
        # Catch any failures (exceptions) and return with None.
            ...
            info('Failed to refresh access token: ' + str(e))
            return None
    ...
    def __save_token_to_cache(self, token):
        ...
        info('OAuth2 flow provided the token below. Cache it to {0}'.format(self.cache_file))
        info(pformat(token, indent = 4))
    ...
    def get_access_token_resource_owner_grant(self, username, password):
        ...
        logging.info('Get a new access token with username and password.')
        scope = DEFAULT_SCOPE
        session.fetch_token(
        ...
        logging.info('OAuth2 flow provided the token below.')
        logging.info(pformat(session.token, indent = 4))
        return session.token['access_token']
